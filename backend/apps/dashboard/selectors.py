...
from incidents.models import Incident, IncidentStatus, IncidentPriority
from sla.models import IncidentSLA
from escalation.models import IncidentEscalation, EscalationStatus, EscalationTriggerType
...
def get_dashboard_summary(organization: Organization) -> dict[str, int]:
    ...
    incident_stats = Incident.objects.filter(organization=organization).aggregate(
        ...
        unassigned_incidents=Count("id", filter=Q(status__in=open_statuses, assignee__isnull=True)),
    )
...
def get_priority_distribution(organization: Organization) -> dict[str, int]:
    ...
    """
    counts = Incident.objects.filter(organization=organization).values("priority").annotate(
        count=Count("id")
    )
...
def get_sla_metrics(organization: Organization) -> dict[str, int]:
    ...
    metrics = annotated_qs.aggregate(
        healthy=Count("id", filter=Q(
            response_breached=False, 
            resolution_breached=False,
            is_resp_at_risk=False,
            is_resol_at_risk=False
        ) & (
            Q(response_completed_at__isnull=True) | Q(resolution_completed_at__isnull=True)
        )),
        approaching=Count("id", filter=Q(is_resp_at_risk=True) | Q(is_resol_at_risk=True)),
        breached=Count("id", filter=Q(response_breached=True) | Q(resolution_breached=True)),
        resolved_within_sla=Count("id", filter=Q(
...
def get_escalation_metrics(organization: Organization) -> dict[str, Any]:
    ...
    level_counts = qs.filter(status=EscalationStatus.ACTIVE).values("current_level").annotate(
        count=Count("id")
    ).order_by("current_level")
    
    summary["by_level"] = {str(item["current_level"]): item["count"] for item in level_counts}
...
def get_dashboard_incidents(
    ...
    if sla_state:
        ...
        if sla_state == "BREACHED":
            qs = qs.filter(Q(sla__response_breached=True) | Q(sla__resolution_breached=True))
            ...
            qs = qs.annotate(
                ...
                (
                    Q(sla__response_completed_at__isnull=True, sla__response_breached=False) & 
                    Q(resp_elapsed__gte=F("resp_total") * threshold)
                ) |
                (
                    Q(sla__resolution_completed_at__isnull=True, sla__resolution_breached=False) & 
                    Q(resol_elapsed__gte=F("resol_total") * threshold)
                )
            )
