...
class DashboardSelectorsTest(TestCase):
    ...
    def test_get_dashboard_summary(self):
        ...
        IncidentService.create_incident(
            self.org, self.user, "Incident 1", priority="P3"
        )

        # 2. Assigned, Open, Breached SLA
        inc2 = IncidentService.create_incident(
            self.org, self.user, "Incident 2", priority="P1"
        )
        ...
        EscalationEvaluationService.evaluate_incident_escalation(
            incident=inc2, trigger_type=EscalationTriggerType.RESPONSE_BREACH
        )

        # 3. At Risk SLA (90% elapsed)
        inc3 = IncidentService.create_incident(
            self.org, self.user, "Incident 3", priority="P1"
        )
        sla3 = inc3.sla
    ...
    def test_get_sla_metrics(self):
        ...
        # Breached
        inc2 = IncidentService.create_incident(
            self.org, self.user, "Breached", priority="P1"
        )
        sla2 = inc2.sla
        sla2.response_deadline = timezone.now() - timedelta(minutes=1)
        sla2.save()
        ...
        # Resolved within SLA
        inc3 = IncidentService.create_incident(
            self.org, self.user, "Resolved", priority="P1"
        )
        IncidentService.transition_incident_status(
            inc3, self.membership, "ACKNOWLEDGED"
        )
    ...
    def test_get_dashboard_incidents_filtering(self):
        inc1 = IncidentService.create_incident(
            self.org, self.user, "Inc 1", priority="P1"
        )
        IncidentService.create_incident(self.org, self.user, "Inc 2", priority="P3")
    ...
    def test_tenant_isolation(self):
        ...
        other_org = OrganizationService.create_organization(
            user=other_user, name="Other Org"
        )
