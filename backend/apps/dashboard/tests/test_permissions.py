...
from organizations.models import Membership, MembershipStatus, Role, Permission
...
class CanViewDashboardPermissionTest(TestCase):
    def setUp(self):
        self.user = User.objects.create_user(email="perm@example.com", password="Password123!")
        self.org = OrganizationService.create_organization(user=self.user, name="Perm Org")
    ...
    def test_agent_has_incident_view_permission(self):
        agent_user = User.objects.create_user(email="agent@perm.test", password="Password123!")
    ...
    def test_viewer_has_incident_view_permission(self):
        viewer_user = User.objects.create_user(email="viewer@perm.test", password="Password123!")
...
class DashboardPermissionAPITest(APITestCase):
    def setUp(self):
        self.owner = User.objects.create_user(email="owner@perm.test", password="Password123!")
        self.no_member = User.objects.create_user(email="nomember@perm.test", password="Password123!")

        self.org = OrganizationService.create_organization(user=self.owner, name="Perm Org")
        self.viewer_user = User.objects.create_user(email="viewer@perm.test", password="Password123!")
    ...
    def test_unauthenticated_denied(self):
        for url in [self.summary_url, self.sla_url, self.escalation_url, self.priority_url, self.incidents_url]:
            res = self.client.get(url)
            self.assertEqual(res.status_code, status.HTTP_401_UNAUTHORIZED, msg=f"Failed for {url}")

    def test_non_member_denied(self):
        self.client.force_authenticate(user=self.no_member)
        for url in [self.summary_url, self.sla_url, self.escalation_url, self.priority_url, self.incidents_url]:
            res = self.client.get(url)
            self.assertEqual(res.status_code, status.HTTP_404_NOT_FOUND, msg=f"Failed for {url}")
    ...
    def test_owner_can_access_all_endpoints(self):
        ...
        for url, code in expected.items():
            res = self.client.get(url)
            self.assertEqual(res.status_code, code, msg=f"Failed for {url}: got {res.status_code}")
    ...
    def test_cross_tenant_returns_404(self):
        other_user = User.objects.create_user(email="other@perm.test", password="Password123!")
        other_org = OrganizationService.create_organization(user=other_user, name="Other Perm Org")
