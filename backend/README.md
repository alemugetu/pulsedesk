...
- \*_`selectors.py` = organization-scoped reads. `SELECT _ FROM ... WHERE organization_id = $1`
...
Channels 4.x removed the `manage.py runserver` WebSocket support, so the ASGI
dev server must be Daphne; plain `manage.py runserver` serves WSGI only and
returns **404 for every `/ws/` path** (this bites every new contributor).
Daphne is declared in `requirements/development.txt`.
...
The 10-item Phase 13.6 security matrix tests enforce this for every security tests for cross-org reads and SettingsSecurityMatrixTest` in apps/settings/tests/test*views.py, plus the same patterns in every app's cross-tenant security tests (see apps/\*/tests/test*\*.py).
